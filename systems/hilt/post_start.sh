#!/bin/sh
# Register piri-0 as hilt's regional provider for us-west-1.
#
...
region="us-west-1"
echo "post_start: registering piri-0 (${piri_did}) as provider for ${region}"
...
if add_err=$(hilt client admin provider add "$piri_did" "$region" 2>&1); then
    :
...
echo "post_start: registered piri-0 as provider for ${region}"
