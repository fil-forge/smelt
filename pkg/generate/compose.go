...
// devModeEnabled reports whether the generator should emit `build:` directives
// in place of (alongside) pulled images. Toggled by `SMELT_DEV=1`. The path is
// resolved relative to generated/compose/piri.yml; siblings of smelt are
// reachable via ../../../<repo>.
func devModeEnabled() bool {
	return os.Getenv("SMELT_DEV") == "1"
}

const piriDevImage = "smelt-local/piri:dev"

// Dev-mode build context is the parent of smelt (resolved relative to
// generated/compose/piri.yml: ../../.. lands at fil-forge/). The Dockerfile
// lives in smelt and copies in piri-pdp + the siblings piri-pdp's go.mod
// `replace ../<name>` points at (libforge, delegator).
const piriDevBuildContext = "../../.."
const piriDevDockerfile = "smelt/dockerfiles/piri.Dockerfile"
...
func buildPiriService(node manifest.ResolvedPiriNode) ComposeService {
	...
	// In dev mode, every piri-N is built from a sibling piri-pdp checkout and
	// shares one local image tag. Per-node manifest images are intentionally
	// ignored here — dev mode assumes you want the working tree.
	var build *BuildSpec
	if devModeEnabled() {
		image = piriDevImage
		build = &BuildSpec{
			Context:    piriDevBuildContext,
			Dockerfile: piriDevDockerfile,
		}
	}
...
// BuildSpec represents a Docker Compose `build:` block.
