...
of a drill run. `setup` runs it before it writes the provider `.env`. `make up`
and `make redeploy` return only after the services they started report
healthy, so a drill can start right after either one.
